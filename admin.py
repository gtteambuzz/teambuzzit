...
class AdminUsersByProject(webapp.RequestHandler):
	def get(self):
		if not testForAdmin(self):
			self.redirect("/admin/login")
		...
		path = os.path.join(os.path.dirname(__file__), 'adminusersbyproject.html')
		self.response.out.write(template.render(path, template_values))
...
Paul Anderson, at panderson@gatech.edu.
...
class Init(webapp.RequestHandler):
	def get(self):
		...
		phases = [
			["pc_apps", datetime.date(2014,9,22), datetime.date(2014,9,29), "PC Application period"],
			["group_create", datetime.date(2014,10,5), datetime.date(2014,10,19), "Group creation period"],
			["group_join", datetime.date(2014,10,5), datetime.date(2014,10,14), "Group joining period"],
			["group_registration", datetime.date(2014,10,5), datetime.date(2014,10,14), "Group registration period (usually start of group creation and end of group joining)"],
			["individual_registration", datetime.date(2014,10,17), datetime.date(2014,10,25), "Individual registration"]
		]
